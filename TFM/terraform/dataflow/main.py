...
def run_pipeline():
    ...
    parser.add_argument('--temp_bucket',
                        dest='temp_bucket',
                        required=False,
                        default='tfm-caixabank-dataflow-staging-tfm',
                        help='GCS bucket for temp and staging files.')
    ...
    print(f"🗂️ Temp/Staging bucket: {args.temp_bucket}")

    # FIXED: Use separate staging bucket for temp/staging, not the input bucket
    ...
    pipeline_options.view_as(GoogleCloudOptions).temp_location = f'gs://{args.temp_bucket}/dataflow-temp/'
    pipeline_options.view_as(GoogleCloudOptions).staging_location = f'gs://{args.temp_bucket}/dataflow-staging/'
