...
def read_and_upload_today_courses_csv_only(**kwargs):
    """
    Fetches today's courses from the API and uploads ONLY CSV to daily_courses/courses_YYYYMMDD.csv
    The GCS_LANDING_BUCKET variable is used as the target bucket.
    """
    ti = kwargs['ti']
    ...
    try:
        ...
        logger.info(f"🪣 Target bucket: {GCS_LANDING_BUCKET}")
    ...
    # 4. Create and upload CSV directly to daily_courses/ folder
    try:
        bucket = client.bucket(GCS_LANDING_BUCKET)
        ...
        logger.info(f"📤 CSV uploaded to gs://{GCS_LANDING_BUCKET}/{csv_filename}")
        logger.info(f"📁 File saved directly in daily_courses/ folder")
        
        # Store path in XCom for potential downstream tasks
        ti.xcom_push(key='gcs_csv_path', value=f"gs://{GCS_LANDING_BUCKET}/{csv_filename}")
    ...
    logger.info(f"   📍 Location: gs://{GCS_LANDING_BUCKET}/{csv_filename}")
    ...
    return {
        ...
        'gcs_path': f"gs://{GCS_LANDING_BUCKET}/{csv_filename}",
...
with DAG(
    ...
    # TESTING: Try without custom service account first
    ...
    move_from_landing_to_processed = GCSToGCSOperator(
        task_id="move_from_landing_to_processed",
        source_bucket=GCS_LANDING_BUCKET,
        source_object=GCS_FILE_NAME,
        destination_bucket=GCS_PROCESSED_BUCKET,
        destination_object=f"daily_courses_processed/{GCS_FILE_NAME}",
        move_object=True,
        retries=RETRIES,
    )
    
    # Task Dependencies
    read_and_upload_today_courses_csv_task >> check_file_on_gcs >> run_dataflow_flex_template >> check_data_on_bigquery >> move_from_landing_to_processed